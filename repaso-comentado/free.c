...
int main(){
    ...
    float* valores = (float*) malloc (longitud * sizeof(float)); //el valor retorna retormnara al puntero valores de tipo float
    //reserva memoria en bytes mediante la operacion longitud * los bytes de un float 
    if(valores  == NULL){ //En caso de no tener memoria lo anterior devolvera NULL por lo que se crea una condicion para que reporte cuando eso pase
        ...
        return 1; //SI SE CUMPLE LA CONDICON ANTERIOR SE TERMINA EL PROGRAMA(reportando que termino mal)
    }
    for (int i = 0; i < longitud; i++){//UN CICLO QUE NOS RECORRERA posiciones segun los parametros
